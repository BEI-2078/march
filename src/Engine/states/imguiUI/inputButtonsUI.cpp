...
{
	static const unsigned numButtonsH = 5, numButtonsV = 4;
	static const char *buttonNames[numButtonsH * numButtonsV] =
	{
		"0",
		"1",
		"2",
	...
	if(ImGui::Begin("inputButtons", nullptr, windowFlags | ImGuiWindowFlags_NoTitleBar))
	{
		...
		ImVec2 buttonSize(inputButtonsUIWidth * 0.23f,  inputButtonsUIHeight* 0.18f);
		...
		*/
		...
		for(unsigned i = 0; i<numButtonsH; i++)
		{
			...
			for(unsigned j = 0; j<numButtonsV; j++)
			{
				...
				{
					//do stuff
					handleButtonAction(activeIndex);
				}
...
void Engine::imguiUI::handleButtonAction(unsigned int buttonIndex)
{
	// Handle the action based on the button index
	switch (buttonIndex)
	{
		case 17:
			this->m_data->m_muController->execute(0);
			break;
		default:
			break;
	}
